...
test_data_images_pth = os.path.join(root, 'test_data_1024', 'images')
test_data_map_pth = os.path.join(root, 'test_data_1024', 'gt_density_map')
test_data_show_pth = os.path.join(root, 'test_data_1024', 'gt_show')
train_data_images_pth = os.path.join(root, 'train_data_1024', 'images')
train_data_map_pth = os.path.join(root, 'train_data_1024', 'gt_density_map')
train_data_show_pth = os.path.join(root, 'train_data_1024', 'gt_show')
...
dota_category = ['plane', 'ship', 'small-vehicle', 'large-vehicle']
...
for pth in img_paths:
    ...
    plane=cv2.distanceTransform(distance_map, cv2.DIST_L2, 5)
    ...
    small_vehicle=cv2.distanceTransform(distance_map, cv2.DIST_L2, 5)
    distance_map = (255 * (1 - kpoint[3, :, :].copy())).astype(np.uint8)
    large_vehicle=cv2.distanceTransform(distance_map, cv2.DIST_L2, 5)

    spatial_mask = np.array([plane, ship, small_vehicle, large_vehicle])
