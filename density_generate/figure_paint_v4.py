...
def feature_test(feature, save_pth, category):
    ...
    save_data = save_data_1 + save_data_2
    save_data = 255 * save_data / np.max(save_data)
...
def VisDrone_category_test(img, bbox, color_buf, VisDrone_category_buf):
    ...
    for item in bbox:
        if VisDrone_category_buf[int(item[5])] == '_':
            continue
        if VisDrone_category_buf[int(item[5])] == '！':
            continue
        if VisDrone_category_buf[int(item[5])] == '@':
            break
...
def draw_bounding_box_on_image(image, xmin, ymin, xmax, ymax,
    ...
    # 绘制Box框
    r = 5
    center_x = math.floor(left+right)/2.0
    center_y = math.floor(top+bottom)/2.0
    draw.rectangle((center_x-r, center_y-r, center_x+r, center_y+r), color, color)
    ...
    # if  str(display_str_list).find('i')>=0:
    #     # 绘制文字背景框
    #     draw.rectangle(
...
VisDrone_category_buf = [ '！', 'people', 'people', '_', 'car', 'car', 'car', '_', '_', 'car', '_', '_']
kernel_size_buf = [8, 8, 8, 8, 8, 8, 8, 8, 8, 8]
# color_buf = [(0, 0, 255), (0, 255, 0), (255, 0, 0),
#              (0, 125, 125), (125, 0, 125), (125, 125, 0),
#              (0, 25, 25), (25, 0, 25), (25, 25, 0),
#              (0, 255, 255), (255, 0, 255), (255, 255, 0),]
color_buf = [(0, 0, 0), (255,165,0), (255,165,0),
             (0, 0, 0), (0, 255, 0), (0, 255, 0),
             (0, 255, 0), (0, 0, 0), (0, 0, 0),
             (0, 255, 0), (0, 0, 0), (0, 0, 0),]


path_sets = [test_label_pth]
img_paths=[]
...
for idx, pth in enumerate(img_paths):
    if idx != 250:
        continue
    ...
    if str(pth).find('train') > 0:
        target_pth = pth.replace('VisDrone2019-DET-train/annotations','train_data/images').replace('.txt','.jpg')
    if str(pth).find('val') > 0:
        target_pth = pth.replace('VisDrone2019-DET-val/annotations','test_data/images').replace('.txt','.jpg')
    ...
    for item in bbox:
        ...
        if (str(VisDrone_category_buf[int(item[5])]).find('people')>=0) | (str(VisDrone_category_buf[int(item[5])]).find('pedestrian')>=0)  :
            ...
            kpoint[int(item[5]) -1, math.floor(center_x), math.floor(center_y)] = 1
    ...
    density_map[3, :, :] = density_map[3, :, :] + density_map[4, :, :] + density_map[5, :, :] + density_map[8, :, :]#将“汽车”，“货车”，“卡车”，“公共汽车”都记作车
    kpoint[3, :, :] = kpoint[3, :, :] + kpoint[4, :, :] + kpoint[5, :, :] + kpoint[8, :, :]#将“汽车”，“货车”，“卡车”，“公共汽车”都记作车
    ...
    ''' density_map_test '''
    feature_test(density_map, '/home/xuwei/projects_wei/MCC/density_generate/test_img', VisDrone_category_buf)
    ...
    #
    # spatial_mask = np.array([person, car])
    #
    # distance = 5
    ...
    #     #hf['kpoint'] = kpoint
    #     hf['density_map'] = density_map[[1,3]]
