...
test_data_images_pth = os.path.join(root, 'test_data_class3', 'images')
test_data_map_pth = os.path.join(root, 'test_data_class3', 'gt_density_map')
test_data_show_pth = os.path.join(root, 'test_data_class3', 'gt_show')
train_data_images_pth = os.path.join(root, 'train_data_class3', 'images')
train_data_map_pth = os.path.join(root, 'train_data_class3', 'gt_density_map')
train_data_show_pth = os.path.join(root, 'train_data_class3', 'gt_show')
...
print('begin convert')
with open("warning_message_class3.txt", "w") as f:
...
for pth in img_paths:
    ...
    if str(pth).find('train') > 0:
        target_pth = pth.replace('VisDrone2019-DET-train/annotations','train_data_class3/images').replace('.txt','.jpg')
    if str(pth).find('val') > 0:
        target_pth = pth.replace('VisDrone2019-DET-val/annotations','test_data_class3/images').replace('.txt','.jpg')
    ...
    for item in bbox:
        ...
        if (str(VisDrone_category_buf[int(item[5])]).find('people')>=0) | (str(VisDrone_category_buf[int(item[5])]).find('pedestrian')>=0)  :
            ...
            left, bottom = resize((source_shape, bottom, left), 2048, 'coordinate')
            right, top = resize((source_shape, top, right), 2048, 'coordinate')
    ...
    pedestrian=cv2.distanceTransform(distance_map, cv2.DIST_L2, 5)
    ...
    spatial_mask = np.array([pedestrian, person, car, motor])
    ...
    with h5py.File(target_pth.replace('images', 'gt_density_map').replace('.jpg', '.h5'), 'w') as hf:
        #hf['kpoint'] = kpoint
        hf['density_map'] = density_map[[1,3,9]]
