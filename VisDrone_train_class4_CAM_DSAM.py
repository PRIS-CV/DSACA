...
from Network.VisDrone_class4 import VGG
...
VisDrone_category = ['pedestrian', 'people', 'car', 'motor']
...
def train(Pre_data, model, criterion, optimizer, epoch, args, scheduler):
    ...
    train_loader = torch.utils.data.DataLoader(
        dataset.listDataset_visdrone_class_4(Pre_data, args.task_id,
                            shuffle=True,
    ...
    for i, (fname, img, target, kpoint, mask_map) in enumerate(train_loader):
        ...
        mask_motor_map = torch.unsqueeze(mask_map[0, 3, :, :], 0)
...
def validate(Pre_data, model, args):
    ...
    for i, (fname, img, target, kpoint, mask_map)  in enumerate(test_loader):
        ...
        mask_pre = torch.cat((mask_pedestrian, mask_people, mask_car, mask_motor), 0)
        ...
        if i%50 == 0:
            ...
            feature_test(source_img, mask_map.data.cpu().numpy(), target.data.cpu().numpy(), mask_pre.data.cpu().numpy(),
                         ...
                         './vision_map/VisDrone_class4/img{}.jpg'.format(str(i)), VisDrone_category)

    mae = mae*1.0 / len(test_loader)
    for idx in range(len(VisDrone_category)):
        mse[idx] = math.sqrt(mse[idx] / len(test_loader))

    print('\n* VisDrone_class4', '\targs.gpu_id:',args.gpu_id )
    ...
    print('* motor_MAE {motor_mae:.3f}  * motor_MSE {motor_mse:.3f}'.format(motor_mae=mae[3], motor_mse=mse[3]))
