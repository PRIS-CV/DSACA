...
from Network.baseline import VGG
...
def train(Pre_data, model, criterion, optimizer, epoch, args, scheduler):
    ...
    for i, (fname, img, target, kpoint, mask_map) in enumerate(train_loader):
        ...
        # else:
        density_map_pre_1 = model(img, target)
...
def validate(Pre_data, model, args):
    ...
    for i, (fname, img, target, kpoint, mask_map)  in enumerate(test_loader):
        ...
        with torch.no_grad():
            density_map_pre = model(img, target)
        ...
        if i %50 == 0:
            ...
            feature_test(source_img, target.data.cpu().numpy(), density_map_pre.data.cpu().numpy(),'./vision_map/rsoc_v4_class2_2048/img{}.jpg'.format(str(i)), VisDrone_category)
    ...
    #'plane', 'ship', 'small_vehicle', 'large_vehicle'
    print('\n* rsoc_v4_class2_2048', '\targs.gpu_id:',args.gpu_id )
